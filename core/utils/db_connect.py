...
class Request:
    ...
    async def add_product(self, product_id, name, quantity):
        query = (f"INSERT INTO {config('DB_NAME')} (id, name, quantity) "
                 f"VALUES ({product_id}, '{name}', {quantity}) "
    ...
    async def withdraw(self, product_id, quantity_to_remove, message: Message):
